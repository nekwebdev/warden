...
- Safe edits: Keep manifest `pi.extensions` at `./extensions/*/index.ts` and package export at `./src/index.ts`. Preserve global pane/action registry via `globalThis`, exact commands `/warden`, `/warden:display`, `/warden:packages`, and no `/warden:settings`.
- Verification: Run `npm test --prefix pi-warden/warden-panel`; broader package check is `mise run test:pi-warden`.
- Sharp edges: Settings writes must preserve unknown root keys and unknown `settings.warden` keys. Display pane uses draft state until Apply; Esc must not write. Package-manager behavior belongs in `extensions/warden-packages/`, not core panel framework.
<!-- warden-map:inject:end -->

## Scope Purpose

`pi-warden/warden-panel/` packages Warden panel behavior for Pi Agent: a shared pane framework plus bundled commands/panes for Display and Packages settings.
...
| `src/index.ts` | Public API barrel | Exports pane registry, panel helpers, command helpers, settings types. |
| `src/registry.ts` | Pane/action registry | Uses `globalThis` symbols so separately loaded packages share registry. |
...
| `extensions/warden-display/` | Display pane extension | Registers pane and `/warden:display`; manages glyph preference. |
...
- Pane IDs must be unique; duplicate IDs throw.
...
- Display pane changes are draft-only until Apply; keep Apply visibility tied to pending changes.
- Esc exits without writing.
