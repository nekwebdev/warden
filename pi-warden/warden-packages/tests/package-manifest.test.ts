...
describe("package pi resources", () => {
	it("declares warden-packages package identity", () => {
		assert.equal(pkg.name, "@nekwebdev/warden-packages");
		assert.equal(pkg.type, "module");
		assert.equal(pkg.scripts?.test, "node scripts/run-tests.mjs");
	});

	it("declares Pi and panel runtime dependencies", () => {
		assert.equal(
	...
	it("advertises the warden-packages extension entrypoint", () => {
	...
	it("package dry-run succeeds", () => {
		...
		assert.match(result.stdout, /@nekwebdev\/warden-packages/);
	});
});
